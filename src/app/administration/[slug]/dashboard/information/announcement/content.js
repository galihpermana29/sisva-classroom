...
export default function AnnouncementsList() {
  ...
  const fetchInfo = async () => {
    try {
      const {
        data: { data },
      } = await AcademicAPI.getAllAnnouncements();
  ...
  }, [search, typeFilter, permissionFilter, sortSettings, infoData]);
  ...
  return (
    <Stack sx={{ height: '100%', width: '100%', p: { xs: 0, lg: 4 } }}>
      <Modal
        ...
        onClose={() => {
          setOpenCreateModal(false), formik.setValues(initialData);
        }}
      >
        <Stack
          component={Paper}
